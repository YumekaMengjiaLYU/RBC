...
def main():
    ...
    if os.path.isfile("participants.tsv"):
        dl.get("participants.tsv")
        dl.unlock("participants.tsv")
    with open("participants.tsv", "w+") as tsv_file:
        tsv_writer = csv.writer(tsv_file, delimiter='\t')
        tsv_writer.writerow(['participant_id', 'subject_id'])

    with open('participants.tsv', 'a') as out_file:
        for i in range (0, len(subjects)):
            tsv_writer = csv.writer(out_file, delimiter="\t")
            tsv_writer.writerow([subjects[i],subjects[i][4:]]) #filling the file with subject IDs
    ...
    if os.path.isfile('problem_fmapjsons.txt'):
        dl.get('problem_fmapjsons.txt')
        dl.unlock('problem_fmapjsons.txt')
    with open('problem_fmapjsons.txt', 'w+'):
        pass
    ...
    for i in range (0, len(fmap_json)):
        ...
        if 'IntendedFor' in data:
            ...
            with open('problem_fmapjsons.txt', 'a') as f:
                f.write(fmap_json[i])
                f.write('\n')


        dl.get(fmap_json[i])
        dl.unlock(fmap_json[i])
